...
Most of the importain information is hidden away in the configurations. Daniel Leving made "configurations.tsv" so we can extract that information. 
...
Using Regular Expressions (regex) isolate the condition and extract the information. 
...
all_genes_qvalue.05 %>% dplyr::select(gene_name) %>% unlist %>% unique() %>% length()
...
I want to see how many gene are in each stress group with different q-values. The rational behind this is to get a rough idea of the DEGs location and frequency.
...
# Finding the genes that appear 50% or more 

Scatter_data_1_filter_G_50 <- Scatter_data_1 %>% filter(percent > .5) 

Scatter_data_1_filter_G_50 %>% arrange(desc(percent))


```
Creating the same figure, but as a bar plot. 
...
```{r, find the names of the overlaps genes}
...
# pheatmap(mat=gene_matrix, cellwidth=7, cellheight=1, color=hmcol, breaks=custombreaks,
...
qval.05_genes <- all_genes %>% filter(!is.na(gene_qvalue) & (gene_qvalue < 0.05)) %>% 
...
```{r attempt to normalize and cluster}
...
```{r attempt to normalize and cluster, fig.width=4, fig.height=69}
# First try scaling by standard deviation only - using apply function to rowwise transform the data
norm_gene_expression_mat <- t(apply(X = gene_expr_matrix, MARGIN = 1, FUN = function(x) {
    x / sd(x, na.rm = T)
})) %>% as.matrix()
...
# dev.off()
# png(file = "StressMicePortal_M-hipp_Hclust.png", width = 4, height = 69, units = "in", res = 1300)
# pheatmap(mat = gene_expr_matrix, cellwidth = 5, cellheight = 0.5, breaks = custombreaks, color = hmcol,
#          cluster_rows = rowclust, fontsize_col = 4, cluster_cols = F, fontsize_row = 0.45, 
...
```{r Arc Gene clusting, eval = F, fig.height= 4}
...
pheatmap(mat = new_gene_expr_matrix, cellwidth = 3, cellheight = 3, breaks = custombreaks, color = hmcol,
         cluster_rows = rowclust, fontsize_col = 4, fontsize_row = 3,
         cluster_cols = T, clustering_distance_cols = "euclidean", clustering_method = "complete",
         main = "  Arc & Ciart only Heatmap - Hclust")
